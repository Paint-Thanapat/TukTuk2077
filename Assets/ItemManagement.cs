...
public class ItemManagement : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D hitObject)
    {
        ...
        if (hitObject.gameObject.name == "Player")
        {
            if (playerControler.isitemfull == 1 && playerControler.PlayerInventory == 1 && Hitpop == 1 && playerControler.CanCollect == 1)
            {
                if (gameObject.GetComponent<SpriteRenderer>().sprite.name == "Pop1")
                {
                    ...
                    idForSlot = 0;
                    idForHolder = 1;
                    playerControler.PlayerInventory = 0;
                    print("Inventory = 0");
                    Destroy(gameObject);
                }
